...
def process_message(user_input, chat_history, waiting_for_id, last_credit_amount):
    ...
    if is_greeting(user_input):
        ...
        response = "Mening  scoring modelim buni hisobladi."
    elif waiting_for_id:
        ...
        if parsed_id is not None:
            ...
            if isinstance(prediction, str):
                ...
                response = f"Scoring modelimiz hisobiga ko'ra sizga bir yil muddatga {prediction:.2f} dollar miqdorida pul bera olamiz."
                waiting_for_id = False
        else:
            response = "ID raqamingizni kiriting (masalan, 10  yoki bir yuzi o'n besh)."
            waiting_for_id = True
    else:
        if is_credit_query(user_input):
            waiting_for_id = True
            response = "Scoring limiti uchun ID raqamingizni kiriting (masalan, 10 yoki bir yuzi o'n besh)."
        else:
